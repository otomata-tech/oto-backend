...
- `PUT /api/upload/{token}` — **réception d'un upload signé out-of-bande** (issue #105), **pas de JWT** : le `{token}` est un jeton HMAC scellant `(sub, org, cible)` + TTL court + usage unique (émis par `oto_upload_url`, module `upload_tokens.py`). L'agent y PUT le contenu brut depuis le disque (`curl --data-binary @fichier`) ; le backend matérialise dans la cible (page Documents ou fichier brut de projet) en **réappliquant** son autz (`ownership.can_access(project, write)`), consomme le jeton (anti-rejeu), et renvoie un **accusé léger** (id + longueur), jamais le body. Évite de faire transiter du gros contenu par le contexte du LLM.
