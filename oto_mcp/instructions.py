...
**Porte ton contexte DANS l'appel, pas dans un bracelet.** L'état de session (org/projet/compte/run actifs posés par `oto_use_*`) n'est PAS fiable et ne persiste pas d'un appel au suivant. Quand une action dépend d'un contexte précis, passe-le EN PARAMÈTRE de l'appel — c'est le moyen fiable et sans état : `org=<id>` (outils `oto_*`/admin — agir dans une org donnée), `project=<id>` (outils `data_*` — résout les `slot:<nom>` et scope l'org du projet), `account=<label>` (connecteur multi-compte, ex. « 2 Zoho » — `oto_connector_identities` liste les labels), `run_id=<id>` (rattacher l'appel à un `run_start`). Les bracelets `oto_use_*` restent un raccourci best-effort dans la conversation, mais dès que ça compte, préfère le paramètre.
