...
def register(mcp: FastMCP) -> None:
    ...
    # --- Lecture ------------------------------------------------------------
    @mcp.tool()
    async def brevoauto_list_automations(ctx: Context) -> dict:
        ...
        `updated_at`. Utilise l'`id` avec `brevoauto_get_automation`.
        """
        return await _api("GET", "/workflow/listing")

    @mcp.tool()
    async def brevoauto_get_automation(ctx: Context, workflow_id: int) -> dict:
    ...
    @mcp.tool()
    async def brevoauto_automation_catalog(ctx: Context, workflow_id: int = 0) -> dict:
        ...
        `internal_action_id`, `action_type`, label — à passer à `brevoauto_add_trigger`
        / `brevoauto_add_step`.
        """
    ...
    # --- Création -----------------------------------------------------------
    @mcp.tool()
    async def brevoauto_create_automation(ctx: Context, name: str,
                                      description: str = "") -> dict:
        """Crée un scénario d'automation VIDE et renvoie `{workflow_id}`.

        Étape 1 du build : ensuite `brevoauto_add_trigger` (porte d'entrée), puis
        `brevoauto_add_step` + `brevoauto_configure_step`, puis `brevoauto_set_status('active')`.
        """
    ...
    @mcp.tool()
    async def brevoauto_add_trigger(ctx: Context, workflow_id: int, trigger_name: str,
        ...
        `brevoauto_automation_catalog` (ex. segment = `contact_match_one_segment`, id 19,
        source `contacts`). La condition fine se règle ensuite via
        `brevoauto_configure_trigger`.
        """
    ...
    @mcp.tool()
    async def brevoauto_add_step(ctx: Context, workflow_id: int, step_type: str,
        ...
        id 18). Crée le nœud SANS sa config (→ `brevoauto_configure_step`).
        """
        body: dict = {
    ...
    # --- Configuration ------------------------------------------------------
    @mcp.tool()
    async def brevoauto_configure_step(ctx: Context, workflow_id: int, step_id: int,
    ...
    @mcp.tool()
    async def brevoauto_configure_trigger(ctx: Context, workflow_id: int,
    ...
    @mcp.tool()
    async def brevoauto_delete_trigger(ctx: Context, workflow_id: int,
    ...
    @mcp.tool()
    async def brevoauto_delete_step(ctx: Context, workflow_id: int, step_id: int) -> dict:
        """Supprime une étape d'un scénario. Renvoie `{status}`."""
        return await _api("DELETE", f"/workflow/{int(workflow_id)}/step",
                          {"step_id": int(step_id)})

    @mcp.tool()
    async def brevoauto_set_status(ctx: Context, workflow_id: int,
