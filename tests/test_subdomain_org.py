...
def _wire(monkeypatch):
    ...
    monkeypatch.setattr(org_store, "list_all_orgs",
                        lambda: [{"id": 35, "name": "movinmotion"},
                                 {"id": 2, "name": "Otomata Admin"}])
    # par défaut : membre de 35 uniquement = "rep" ; "other" membre de rien.
    monkeypatch.setattr(roles, "is_org_member",
                        lambda sub, oid: sub == "rep" and oid == 35)
    monkeypatch.setattr(org_store, "get_active_org", lambda sub: 99)
    yield


def test_slug_parsing():
    assert subdomain_org._slug_from_host("movinmotion--mcp.oto.ninja") == "movinmotion"
    assert subdomain_org._slug_from_host("MOVINMOTION--mcp.oto.ninja:443") == "movinmotion"
...
def test_org_resolution_and_cache():
    assert subdomain_org.org_id_for_host("movinmotion--mcp.oto.ninja") == 35
    assert subdomain_org.org_id_for_host("inconnu--mcp.oto.ninja") is None
    assert subdomain_org._CACHE == {"movinmotion": 35}  # seuls les hits cachés


def test_current_org_pins_member(monkeypatch):
    tok = session_org.set_subdomain_cv(35)
    try:
        assert access.current_org("rep") == 35
    finally:
...
def test_current_org_non_member_falls_back(monkeypatch):
    tok = session_org.set_subdomain_cv(35)
    try:
        assert access.current_org("other") == 99   # repli maison, jamais 35
    finally:
        session_org.reset_subdomain_cv(tok)
...
def test_hard_lock_session_override_ignored(monkeypatch):
    ...
    tok = session_org.set_subdomain_cv(35)
    try:
        assert access.current_org("rep") == 35   # précédence du sous-domaine
    finally:
        session_org.reset_subdomain_cv(tok)
...
def test_current_group_locked_to_org(monkeypatch):
    ...
    monkeypatch.setattr(group_store, "get_group",
                        lambda gid: {"id": 2, "org_id": 35} if gid == 2 else None)
    tok = session_org.set_subdomain_cv(35)
    try:
