...
def test_le_compte_garde_son_code_de_production():
    """`unipile` reste ce qu'il est en prod — carte hébergée, flux multi-canal,
    label, modules. Le split ne lui retire QUE ses six namespaces de canal : un
    namespace n'appartient qu'à un connecteur, c'est l'unique geste inévitable."""
    con = providers.REGISTRY["unipile"]
...
def test_chaque_namespace_va_a_son_connecteur():
    ...
    canal. La cohabitation `linkedin_unipile` / `linkedin_aiark` (deux préfixes, deux
    connecteurs) est tenue par `tests/test_linkedin.py`."""
    for canal in CANAUX:
